...
double err_calculation(const double* n1, const  double* n2, int size, int max_threads){
    double err = 0.0;
    #pragma omp parallel for proc_bind(spread)
...
int main() {
    ...
    cout << "err_euler = " << err_calculation(res, out, N1*N2*(N3/2+1),max_threads) << endl;

    flag = true;
    Teplo_2(u_start,u_out,F_complex, alpha_x, alpha_y, alpha_z, Nt, tau, L_x, N1, N2, N3, flag);  
    fftw_execute(bplan_c2r);
    fftw_execute(bplan_r2r);
    cout << "err_rungeKutta4 = " << err_calculation(res, out, N1*N2*(N3/2+1), max_threads) << endl;
