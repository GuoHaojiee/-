...
void f_der(double u_real,double u_imaginary, double t, fftw_complex F_complex,fftw_complex result, double alpha, int k_x, int k_y, int k_z) {
    result[0] = -alpha * alpha * u_real * (k_x * k_x + k_y * k_y + k_z * k_z) + F_complex[0] * f(t);
    result[1] = -alpha * alpha * u_imaginary * (k_x * k_x + k_y * k_y + k_z * k_z) + F_complex[1] * f(t);
}
        ...
        for (int i = 0; i < nx; i++) {
            for (int j = 0; j < ny; j++) {
                for (int k = 0; k < (nz / 2 + 1); k++) {
                    ...
                    f_der(u_next[index][0],u_next[index][1], (n-1) * tau , F_complex[index], K1, alpha, k_x, k_y, k_z);
                    f_der(u_next[index][0]+ tau / 2 * K1[0],u_next[index][1]+ tau / 2 * K1[1], (n-1) * tau + tau / 2, F_complex[index], K2, alpha, k_x, k_y, k_z);
                    f_der(u_next[index][0]+ tau / 2 * K2[0],u_next[index][1]+ tau / 2 * K2[1], (n-1) * tau + tau / 2, F_complex[index], K3, alpha, k_x, k_y, k_z);
                    f_der(u_next[index][0]+ tau *K3[0],u_next[index][1]+ tau / 2 * K3[1], (n-1) * tau + tau, F_complex[index], K4, alpha, k_x, k_y, k_z);
...
void Teplo_2(fftw_complex* u_start, fftw_complex* u_out, fftw_complex* F_complex, double alpha, int N, double tau, double L, int nx, int ny, int nz, bool flag) {
    ...
    if (flag == true)
        rungeKutta(u_out, F_complex, alpha, N, tau, L, nx, ny, nz, f_der);
    else
