...
double u(double x, double y, double z, double t) {
    return (t*t+1)*sin(x)*cos(y)*sin(2*M_PI*z);
}
        ...
        for (int i = 0; i < nx; i++) {
            for (int j = 0; j < (ny/2+1); j++) {
                for (int k = 0; k < (nz/2-1); k++) {
                    ...
                    int k_z = k;
                    u_next[index][0] = u_next[index][0] + tau *(u_next[index][0] * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][0] * f((n-1) *tau));
                    u_next[index][1] = u_next[index][1] + tau *(u_next[index][1] * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][1] * f((n-1) *tau));
                }
            }
        }
    }
        ...
        for (int i = 0; i < nx; i++) {
            for (int j = 0; j <(ny/2+1); j++) {
                for (int k = 0; k < (nz/2-1); k++) {
                    ...
                    int k_y = j <= ny/2 ? j : j -ny;
                    int k_z = k;
                    K1[0] = u_next[index][0] * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][0] * f((n-1) *tau);
                    K1[1] = u_next[index][1] * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][1] * f((n-1) *tau);
                    K2[0] = (u_next[index][0] + tau / 2 * K1[0]) * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][0] * f((n-1) * tau + tau / 2);
                    K2[1] = (u_next[index][1] + tau / 2 * K1[1]) * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][1] * f((n-1) * tau + tau / 2);
                    K3[0] = (u_next[index][0] + tau / 2 * K2[0]) * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][0] * f((n-1) * tau + tau / 2);
                    K3[1] = (u_next[index][1] + tau / 2 * K2[1]) * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][1] * f((n-1) * tau + tau / 2);
                    K4[0] = (u_next[index][0] + tau * K3[0]) * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][0] * f((n-1) * tau + tau);
                    K4[1] = (u_next[index][1] + tau * K3[1]) * (-alpha_x * alpha_x *k_x * k_x -alpha_y * alpha_y *k_y * k_y -alpha_z * alpha_z * (k_z+1) * (k_z+1)) + F_complex[index][1] * f((n-1) * tau + tau);
...
int main() {
    ...
    for(int i = 0; i < N1; ++i) {
        for(int j = 0; j < N2; ++j) {
            for(int k = 1; k <= (N3/2-1); ++k) {
                int index = (i*N2+j)*(N3/2-1)+k-1;
