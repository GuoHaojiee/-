...
double u(double x, double y, double z, double t) {
    return (t+1)*sin(x)*cos(y)*cos(2*M_PI*z);
}
...
double f_(double x, double y, double z, double t) {
    return ((4*M_PI*M_PI+2)*t + 4*M_PI*M_PI+3)*sin(x)*cos(y)*cos(2*M_PI*z);
}
        ...
        for (int i = 0; i < N1; ++i){
            for (int j = 0; j < (N2 / 2 + 1); ++j){
                for (int k = 0; k < n3; ++k){
                    ...
                    u_next[index][0] = u_prev[index][0] + tau *((-1)* k_x * k_x * u_prev[index][0] + (-1)* k_y * k_y * u_prev[index][0] + -k*k*alpha*alpha * u_prev[index][0]) + tau * f_k[index][0];
                    u_next[index][1] = u_prev[index][1] + tau *((-1)* k_x * k_x * u_prev[index][1] + (-1)* k_y * k_y * u_prev[index][1] + -k*k*alpha*alpha * u_prev[index][1]) + tau * f_k[index][1];
                   }
            }
        }
