...
void compute_laplace(fftw_complex* V1_c_, fftw_complex* V2_c_, fftw_complex* V3_c_
                    ,fftw_complex* laplaceV1_c_, fftw_complex* laplaceV2_c_, fftw_complex* laplaceV3_c_, int nx, int ny, int nz)
{
    ...
    for(i = 0; i < nx; ++i) {
        for(j = 0; j < (ny/2+1); ++j) {
            for(k = 0; k < (nz/2+1); ++k) {
                ...
                laplaceV1_c_[index][0] = -V1_c_[index][0] * (k_x * k_x + k_y * k_y + k_z * k_z);
                laplaceV1_c_[index][1] = -V1_c_[index][1] * (k_x * k_x + k_y * k_y + k_z * k_z);    
                laplaceV2_c_[index][0] = -V2_c_[index][0] * (k_x * k_x + k_y * k_y + k_z * k_z);
                laplaceV2_c_[index][1] = -V2_c_[index][1] * (k_x * k_x + k_y * k_y + k_z * k_z);      
            }
            ...
            for(k = 0; k < (nz/2-1); ++k) {
                ...
                laplaceV3_c_[index][0] = -V3_c_[index][0] * (k_x * k_x + k_y * k_y + (k_z+1) * (k_z+1));
                laplaceV3_c_[index][1] = -V3_c_[index][1] * (k_x * k_x + k_y * k_y + (k_z+1) * (k_z+1));
            }
        }
    }
}

void compute_grad(fftw_complex* p1_c_,fftw_complex* gradp1_c_, fftw_complex* gradp2_c_, fftw_complex* gradp3_c_, int nx, int ny, int nz)
{
    int i, j, k, index,k_x,k_y,k_z,index1,index2;
    for(i = 0; i < nx; ++i) {
        for(j = 0; j < (ny/2+1); ++j) {
            for (k = 0; k < (nz/2+1); ++k)
            {
                ...
                k_y = j <= ny / 2 ? j : j - ny;
                k_z = k;
                gradp1_c_[index][0] = -p1_c_[index][1] * k_x;
                gradp1_c_[index][1] = p1_c_[index][0] * k_x;
                gradp2_c_[index][0] = -p1_c_[index][1] * k_y;
                gradp2_c_[index][1] = p1_c_[index][0] * k_y;
            }
        }
    }
    ...
    for(i = 0; i < nx; ++i) {
        for(j = 0; j < (ny/2+1); ++j) {
            for(k = 0; k < (nz/2-1); ++k) {
                ...
                k_y = j <= ny/2 ? j : j -ny;
                k_z = k;
                gradp3_c_[index2][0] = -p1_c_[index1][0] * (k_z+1);
                gradp3_c_[index2][1] = -p1_c_[index1][1] * (k_z+1); 
            }
        }
    }
}
...
void compute_p(fftw_complex* F1_c_, fftw_complex* F2_c_, fftw_complex* F3_c_, fftw_complex* p_c_, int nx, int ny, int nz1, int nz2,double L_x, double L_y, double L_z)
{
    int i, j, k,k_x,k_y,k_z,index1,index2;
    double alpha = 1;
    for(i = 0; i < nx; ++i) {
        for(j = 0; j < (ny/2+1); ++j) {
            for(k = 0; k < nz1; ++k) {
                ...
                {
                    p_c_[index1][0] = ((-F1_c_[index1][1] * k_x) + (-F2_c_[index1][1] * k_y) + (F3_c_[index2][0] * k_z*alpha)) / -(k_x*k_x + k_y*k_y + k_z*k_z*alpha*alpha);
                    p_c_[index1][1] = ((F1_c_[index1][0] * k_x) +  (F2_c_[index1][0] * k_y ) + (F3_c_[index2][1] * k_z*alpha)) / -(k_x*k_x + k_y*k_y + k_z*k_z*alpha*alpha); 
                } 
                else if (k == 0 || k == nz1 - 1) 
                {
                    p_c_[index1][0] = (-F1_c_[index1][1] * k_x) + (-F2_c_[index1][1] * k_y)/ -(k_x*k_x + k_y*k_y + k_z*k_z*alpha*alpha);
                    p_c_[index1][1] = (F1_c_[index1][0] * k_x) +  (F2_c_[index1][0] * k_y )/ -(k_x*k_x + k_y*k_y + k_z*k_z*alpha*alpha);
                }
            }
        }
    }
...
{   
    ...
    fftw_complex *laplaceV1_c = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nx * (ny/2+1) * nz1);
    fftw_complex *laplaceV2_c = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nx * (ny/2+1) * nz1);
    fftw_complex *laplaceV3_c = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nx * (ny/2+1) * nz2);
    ...
    fftw_complex *gradp1_c = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nx * (ny/2+1) * nz1);
    fftw_complex *gradp2_c = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nx * (ny/2+1) * nz1);
    fftw_complex *gradp3_c = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nx * (ny/2+1) * nz2);
    // f
    ...
    for (int l = 0; l < nt; l++) {
        ...
        for(i = 0; i < nx; ++i) {
            for(j = 0; j < (ny/2+1); ++j) {
                ...
                for (k = 0; k < nz2; ++k)
                {
                    index = (i*(ny/2+1)+j)*nz2+k;
                    V3_out[index][0] = V3_start[index][0];
                    V3_out[index][1] = V3_start[index][1];
                }
            }
        }
        compute_laplace(V1_start,V2_start,V3_start,laplaceV1_c,laplaceV2_c,laplaceV3_c,nx,ny,Nz);
        ...
        compute_f(f1_c,f2_c,f3_c,f1_z_r,f2_z_r,f3_z_r,f1_r,f2_r,f3_r,nx,ny,nz1,nz2,Nz,L_x,L_y,L_z,nt,tau,l);
        ...
        for(i = 0; i < nx; ++i) {
            for(j = 0; j < (ny/2+1); ++j) {
                ...
                {   
                    index = (i*(ny/2+1)+j)*nz1+k;
                    F1_c[index][0] = laplaceV1_c[index][0] + cross1_c[index][0] + f1_c[index][0];
                    F1_c[index][1] = laplaceV1_c[index][1] + cross1_c[index][1] + f1_c[index][1];
                    F2_c[index][0] = laplaceV2_c[index][0] + cross2_c[index][0] + f2_c[index][0];
                    F2_c[index][1] = laplaceV2_c[index][1] + cross2_c[index][1] + f2_c[index][1];
                }
                for (k = 0; k < nz2; ++k)
                {
                    index = (i*(ny/2+1)+j)*nz2+k;
                    F3_c[index][0] = laplaceV3_c[index][0] + cross3_c[index][0] + f3_c[index][0];
                    F3_c[index][1] = laplaceV3_c[index][1] + cross3_c[index][1] + f3_c[index][1];
                }
            }
        }
        ...
        if (l >= 1)
        {
            compute_p(F1_c, F2_c, F3_c, p_c, nx, ny, nz1, nz2, L_x, L_y, L_z);
        }

        compute_grad(p_c,gradp1_c,gradp2_c,gradp3_c,nx,ny,Nz);
        ...
        for(i = 0; i < nx; ++i) {
            for(j = 0; j < (ny/2+1); ++j) {
                ...
                {   
                    index = (i*(ny/2+1)+j)*nz1+k;
                    V1_out[index][0] = V1_out[index][0] + tau * (laplaceV1_c[index][0] + cross1_c[index][0] + f1_c[index][0] - gradp1_c[index][0]);
                    V1_out[index][1] = V1_out[index][1] + tau * (laplaceV1_c[index][1] + cross1_c[index][1] + f1_c[index][1] - gradp1_c[index][1]);
                    V2_out[index][0] = V2_out[index][0] + tau * (laplaceV2_c[index][0] + cross2_c[index][0] + f2_c[index][0] - gradp2_c[index][0]);
                    V2_out[index][1] = V2_out[index][1] + tau * (laplaceV2_c[index][1] + cross2_c[index][1] + f2_c[index][1] - gradp2_c[index][1]);
                }
                for (k = 0; k < nz2; ++k)
                {
                    index = (i*(ny/2+1)+j)*nz2+k;
                    V3_out[index][0] = V3_out[index][0] + tau * (laplaceV3_c[index][0] + cross3_c[index][0] + f3_c[index][0] - gradp3_c[index][0]);
                    V3_out[index][1] = V3_out[index][1] + tau * (laplaceV3_c[index][1] + cross3_c[index][1] + f3_c[index][1] - gradp3_c[index][1]);
                }
            }
        }
    ...
    fftw_free(laplaceV1_c);
    fftw_free(laplaceV2_c);
    fftw_free(laplaceV3_c);
    fftw_free(rot1_c);
    fftw_free(rot2_c);
    fftw_free(rot3_c);
    fftw_free(rot1_xy_r);
    ...
    for (i = 0; i < nx; i++) {
        for (j = 0; j < ny; j++) {
            for (k = 0; k < nz1; k++) {
                ...
                err1_ = fabs(V1_out_r[index1] - func_V1(i*L_x/nx, j*L_y/ny, k*L_z/Nz,nt*tau));
                err2_ = fabs(V2_out_r[index1] - func_V2(i*L_x/nx, j*L_y/ny, k*L_z/Nz,nt*tau));
                err1 += err1_ * err1_;
                err2 += err2_ * err2_;
            }
            for (k = 1; k <= nz2; k++) {
                int index2 = (i*ny+j)*nz2+k-1;
                err3_ = fabs(V3_out_r[index2] - func_V3(i*L_x/nx, j*L_y/ny, k*L_z/Nz,nt*tau));
                err3 += err3_ * err3_;           
            }     
        }  
    }
